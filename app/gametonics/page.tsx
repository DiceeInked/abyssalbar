...
export default function GameTonics() {
  ...
  useEffect(() => {
    const loadAccount = async () => {
      ...
      catch (loadError) { console.error("Error loading account session:", loadError); writeCommand("authentication error: unable to load session."); }
    };
    void loadAccount();
  }, []);
  ...
  const authenticate = async (action: "sign_up" | "sign_in", username: string, password: string) => {
    ...
    setBusy(true);
    try { const response = await fetch("/api/auth", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, username, password }) }); const result = await response.json(); if (!response.ok) return void writeCommand(`${action === "sign_up" ? "sign up" : "sign in"} error: ${result.error}`); setAccount(result.account); writeCommand(action === "sign_up" ? `account created. welcome, ${result.account.username}!` : `signed in. welcome back, ${result.account.username}!`); }
    catch (authError) { console.error("Authentication error:", authError); writeCommand("authentication error: unable to contact the server."); }
    finally { setBusy(false); }
  };
  const signOut = async () => {
    setBusy(true);
    try { const response = await fetch("/api/auth", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "sign_out" }) }); const result = await response.json(); if (!response.ok) return void writeCommand(`sign out error: ${result.error}`); setAccount(null); writeCommand("signed out."); }
    catch (signOutError) { console.error("Sign out error:", signOutError); writeCommand("sign out error: unable to contact the server."); }
    finally { setBusy(false); }
  };
  const runCommand = async (value: string) => {
    ...
    switch (command) {
      ...
      case "/sign": { const action = args[0]?.toLowerCase(); if (action === "up" && args.length === 3) return void await authenticate("sign_up", args[1], args[2]); if (action === "in" && args.length === 3) return void await authenticate("sign_in", args[1], args[2]); if (action === "out" && args.length === 1) return void await signOut(); writeCommand("usage: /sign up <username> <password>, /sign in <username> <password>, or /sign out"); return; }
