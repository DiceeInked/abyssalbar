...
export default function Etho() {
  ...
  const [bootLoadingText, setBootLoadingText] = useState("INSERT TAPE");
  ...
  const insertDvd = () => {
    ...
    setBootLoadingText("INSERTING TAPE...");
    ...
    window.setTimeout(() => {
      setBootLoadingText("READING TAPE...");
  ...
  useEffect(() => {
    ...
    const handleEnded = () => {
      ...
      setBootAudioError(false);
      addLine("warning", "[ TAPE AUDIO COMPLETE // ECHO TERMINAL READY ]");
    };
  ...
  useEffect(() => {
    ...
    const initialX = Math.max(
      12,
      Math.min(window.innerWidth / 2 - width / 2, window.innerWidth - width - 12),
    );
  ...
  useEffect(() => {
    if (!warningOpen && !typing && !dessMode && !proceedMenuOpen && !finalScene) {
  ...
  const makeMusicUnsettling = (intensity = 1) => {
    ...
    try {
      if (!audioContext.current) {
        ...
        const impulse = context.createBuffer(2, Math.floor(context.sampleRate * 2.6), context.sampleRate);
  ...
  const addFaultWindow = () => {
    setErrorWindows((current) =>
      current.length >= 60 ? current : [...current, createErrorWindow(FAULT_MESSAGES)],
    );
  };
  ...
  const openProceedDialog = () => {
    ...
    proceedFadeTimer.current = null;
    if (proceedCount === 0) {
      musicWasPlayingBeforeProceed.current = Boolean(music.current && !music.current.paused);
    }
    setProceedChoice("proceed");
  ...
  }, [proceedChoice, proceedMenuOpen]);

  const dismissError = (id: number) => {
    setErrorWindows((current) => current.filter((item) => item.id !== id));
    if (!faultMode) return;
        ...
        {!dessMode && !finalScene && (
          <>
            ...
            <EthoTerminalView
              ...
              typing={typing}
              dessMode={dessMode}
              proceedMenuOpen={proceedMenuOpen}
              finalScene={finalScene}
              terminalRef={terminalRef}
              inputRef={inputRef}
              activeProceedPrompt={activeProceedPrompt}
              proceedChoice={proceedChoice}
              whiteoutLevel={whiteoutLevel}
              proceedLineCount={PROCEED_LINES.length}
              onInputChange={setInput}
              onSubmit={handleSubmit}
              onProceedChoiceChange={setProceedChoice}
              onResolveProceed={resolveProceedChoice}
